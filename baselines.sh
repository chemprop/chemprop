...
for i in ${!datasets[@]}; do
    ...
    for fold in ${!folds[@]}; do
        ...
        if [[ "${split_type}" == "random" ]]; then
            ...
            file="/data/rsg/chemistry/yangk/data/${datasets[$i]}/scaffold/fold_$i/0/split_indices.pckl"
        ...
        if [[ ! -e "$file" ]]; then
            ...
            CUDA_VISIBLE_DEVICES=${gpus[${gpu_index}]} python train.py --data_path data/${datasets[$i]}.csv --dataset_type ${dataset_type[$i]} --save_dir ../ckpt/417_ffn_morgan/${datasets[$i]}/${split_type}/${folds[$fold]} ${split_info} --features_only --features_generator morgan --quiet --metric ${metrics[$i]} &
            CUDA_VISIBLE_DEVICES=${gpus[${gpu_index}]} python train.py --data_path data/${datasets[$i]}.csv --dataset_type ${dataset_type[$i]} --save_dir ../ckpt/417_ffn_morgan_count/${datasets[$i]}/${split_type}/${folds[$fold]} ${split_info} --features_only --features_generator morgan_count --quiet --metric ${metrics[$i]} &
            CUDA_VISIBLE_DEVICES=${gpus[${gpu_index}]} python train.py --data_path data/${datasets[$i]}.csv --dataset_type ${dataset_type[$i]} --save_dir ../ckpt/417_ffn_rdkit/${datasets[$i]}/${split_type}/${folds[$fold]} ${split_info} --features_only --features_path /data/rsg/chemistry/yangk/saved_features/${datasets[$i]}.pckl --no_features_scaling --quiet --metric ${metrics[$i]} &
