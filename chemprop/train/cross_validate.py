...
def cross_validate(
    ...
    for fold_num in range(args.num_folds):
        for metric, scores in all_scores.items():
            info(
                f"\tSeed {init_seed + fold_num} ==> test {metric} = {multitask_mean(scores[fold_num], metric):.6f}"
            )
    ...
    if contains_nan_scores:
        info(
            "The metric scores observed for some fold test splits contain 'nan' values. \
    ...
    if args.save_preds:
        all_preds = pd.concat(
            [
                pd.read_csv(os.path.join(save_dir, f"fold_{fold_num}", "test_preds.csv"))
                for fold_num in range(args.num_folds)
            ]
        )
