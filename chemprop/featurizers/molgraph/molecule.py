...
@dataclass
class PretrainMoleculeMolGraphFeaturizer(_MolGraphFeaturizerMixin, GraphFeaturizer[Mol]):
    """A :class: PretrainMoleculeMolGraphFeaturizer contains additional useful information than the default
    ...
    def __call__(
        ...
        if n_atoms == 0:
            ...
            V = np.array([self.atom_featurizer(a) for a in mol.GetAtoms()], dtype=np.single)
            atom_num_label = [atom.GetAtomicNum() for atom in mol.GetAtoms()]
        E = np.empty((2 * n_bonds, self.bond_fdim))
        edge_index = [[], []]
