...
def main(args):
    ...
    for fold_idx in range(args.num_folds):
        if multicomponent:
            ...
            logger.info(f"train/val/test sizes: {len(train_data[fold_idx])}/{len(val_data[fold_idx])}/{len(test_data[fold_idx])}")
        ...
        for model_idx in range(args.ensemble_size):
            if no_cv:
                ...
                logger.info(f"Training model {model_idx + 1}/{args.ensemble_size} for fold {fold_idx + 1}/{args.num_folds}")
            ...
            if no_cv:
                ...
                output_dir = args.output_dir / f"fold_{fold_idx}" / f"model_{model_idx}"
