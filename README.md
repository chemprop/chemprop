...
* **Kennard-Stone** Strictly enforces model interpolation during training to better assess capacity for inference. Useful for high-dimensional and/or small datasets, this algorithm attempts to sample feature space uniformly into the training set. Usage: `--split_type kennard_stone`
* **KMeans** Performs clustering on the Morgan fingerprint representation of the molecules and then assigns separate clusters to training, validation, and testing. Useful for rigorously testing model extrapolation ability. Usage: `--split_type kmeans`
