...
for i in ${!datasets[@]}; do
    ...
    for fold in ${!folds[@]}; do
        ...
        if [ ! -e "$file" ]; then
            echo "Fold indices do not exist"
        else 
            CUDA_VISIBLE_DEVICES=$gpu python hyperparameter_optimization.py --data_path data/${datasets[$i]}.csv --dataset_type ${dataset_type[$i]} --split_type crossval --crossval_index_file crossval_index_files/${sizes[$i]}/${folds[$fold]}.pkl --crossval_index_dir crossval_folds/${datasets[$i]}/random --features_path /data/rsg/chemistry/yangk/saved_features/${datasets[$i]}.pckl --no_features_scaling --num_iters 30 --config_save_path ../ckpt/417_hyperopt/${datasets[$i]}/random/${folds[$fold]}/config.json --log_dir ../ckpt/417_hyperopt/${datasets[$i]}/random/${folds[$fold]}/logdir --quiet --metric ${metrics[$i]} &
        fi 
        gpu=$((($gpu + 1) % $num_gpus))
    done
    wait
