...
for i in ${!datasets[@]}; do
    ...
    for fold in ${!folds[@]}; do
        ...
        if [[ "${split_type}" == "random" ]]; then
            ...
            file="/data/rsg/chemistry/yangk/lsc_experiments_dump_splits/data/${datasets[$i]}/scaffold/fold_$fold/opt_split_indices.pckl"
            split_info="--split_type index_predetermined --crossval_index_file $file"
        fi
        if [[ ! -e "$file" ]]; then
