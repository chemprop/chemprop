...
@patch("chemprop.data.utils.get_header", lambda *args: [f"column{i}" for i in range(5)])
...
class TestPreprocessSmiles(TestCase):
    ...
    def test_basecase_1mol(self):
        ...
        smiles_columns = preprocess_smiles_columns(
            path="dummy_path.txt", smiles_columns=None, number_of_molecules=1
        )
    ...
    def test_2mol(self):
        ...
        smiles_columns = preprocess_smiles_columns(
            path="dummy_path.txt", smiles_columns=None, number_of_molecules=2
        )
    ...
    def test_specified_smiles(self):
        ...
        smiles_columns = preprocess_smiles_columns(
            path="dummy_path.txt", smiles_columns=["column3"], number_of_molecules=1
        )
    ...
    def test_out_of_order_smiles(self):
        ...
        smiles_columns = preprocess_smiles_columns(
            path="dummy_path.txt", smiles_columns=["column3", "column2"], number_of_molecules=2
        )
    ...
    def test_wrong_num_mol(self):
        ...
        with self.assertRaises(ValueError):
            smiles_columns = preprocess_smiles_columns(
                path="dummy_path.txt", smiles_columns=["column3"], number_of_molecules=2
            )
...
class TestGetTaskNames(TestCase):
    ...
    def test_default_no_arguments(self):
        ...
        task_names = get_task_names(
            path="dummy_path.txt", smiles_columns=None, target_columns=None, ignore_columns=None
        )
...
class TestGetSmiles(TestCase):
    ...
    def test_specified_columns_changed_order(self):
        """Testing with no optional arguments."""
        smiles = get_smiles(path=self.smiles_path, smiles_columns=["column1", "column0"])
    ...
    def test_noheader_2mol(self):
        ...
        smiles = get_smiles(path=self.no_header_path, number_of_molecules=2, header=False)
        self.assertEqual(smiles, [["C", "CC"], ["CC", "CN"], ["O", "CO"]])

    def test_flatten(self):
        """Testing with flattened output"""
        smiles = get_smiles(path=self.smiles_path, flatten=True)
        self.assertEqual(smiles, ["C", "CC", "CC", "CN", "O", "CO"])
...
@patch("chemprop.data.utils.get_data_weights", lambda *args, **kwargs: np.array([1, 1.5, 0.5]))
@patch(
    "chemprop.data.utils.preprocess_smiles_columns",
    lambda *args, **kwargs: ["column0", "column1"],  # default smiles columns if unspecified
)
class TestGetData(TestCase):
    ...
    def test_features(self):
        ...
        data = get_data(path=self.data_path, features_path=["dummy_path.csv"])
        print(data.features())
    ...
    def test_2features(self):
        ...
        data = get_data(
            path=self.data_path, features_path=["dummy_path.csv", "also_dummy_path.csv"]
        )
        print(data.features())
    ...
    def test_dataweights(self):
        ...
        data = get_data(path=self.data_path, data_weights_path="dummy_path.csv")
    ...
    def test_phase_features(self):
        ...
        data = get_data(path=self.data_path, phase_features_path="dummy_path.csv")
