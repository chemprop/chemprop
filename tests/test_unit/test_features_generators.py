...
TEST_MULTIMOLECULE_LIST = [[TEST_SMILES_STRING, TEST_SMILES_STRING], [TEST_SMILES_STRING, TEST_SMILES_STRING]]
...
class TestFeatureGenerators:
    class TestOutputDimensionality:
        @pytest.mark.parametrize("mol_data, radius, num_bits",
        ...
        def test_morgan(self, mol_data, radius, num_bits):
            ...
            features = features_generators.morgan_binary_features_generator(mol_data, radius, num_bits)

            if isinstance(mol_data, Sequence):
                if isinstance(mol_data[0], Sequence):
                    assert features.shape == (len(mol_data), len(mol_data[0]), MORGAN_NUM_BITS)
                else:
        ...
        def test_morgan_counts(self, mol_data, radius, num_bits):
            ...
            features = features_generators.morgan_counts_features_generator(mol_data, radius, num_bits)
        ...
        @pytest.mark.parametrize("mol_data",
                                 [TEST_SMILES_STRING,
                                  TEST_MOLECULE_OBJECT,
        ...
        def test_rdkit2d(self, mol_data):
            ...
            if isinstance(mol_data, Sequence):
                if isinstance(mol_data[0], Sequence):
                    assert features.shape == (len(mol_data), len(mol_data[0]), NUM_RDKIT_FEATURES)
                else:
        ...
        def test_rdkit2d_normalized(self, mol_data):
            ...
            features = features_generators.rdkit_2d_normalized_features_generator(mol_data)
    ...
    class TestGeneratorRegistration:
        def test_default_generator_registration(self):
            ...
            assert features_generators.get_available_features_generators() == ['morgan', 'morgan_count', 'rdkit_2d', 'rdkit_2d_normalized']
        ...
        def test_custom_generator_registration(self):
            ...
            assert 'custom_generator' in features_generators.get_available_features_generators()
