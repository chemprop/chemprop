...
for alpha in alpha_list:
    ...
    os.system(f"python3 ../predict.py --test_path data/{file_name}.csv --checkpoint_dir quantile_regression/delaney_checkpoints_{file_name}_quantile_{alpha} --preds_path quantile_regression_new/{file_name}_preds_conformal_quantile_{alpha}.csv --calibration_method conformal_quantile_regression --calibration_path data/{file_name}.csv --conformal_alpha {alpha} --evaluation_methods conformal_coverage --evaluation_scores_path quantile_regression_new/{file_name}_eval_conformal_{alpha}.csv")
