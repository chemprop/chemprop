...
for i in ${!datasets[@]}; do
    ...
    for fold in ${!folds[@]}; do
        ...
        if [[ ! -e "$file" ]]; then
            ...
            CUDA_VISIBLE_DEVICES=${gpus[${gpu_index}]} python train.py --data_path data/${datasets[$i]}.csv --dataset_type ${dataset_type[$i]} --save_dir ../ckpt/417_hyperopt_eval/${datasets[$i]}/random/${folds[$fold]} ${split_info} --features_path /data/rsg/chemistry/yangk/saved_features/${datasets[$i]}.pckl --no_features_scaling --config_path ../ckpt/417_hyperopt/${datasets[$i]}/random/${folds[$fold]}/config.json --quiet --metric ${metrics[$i]} &
...
for i in ${!datasets[@]}; do
    ...
    for fold in ${!folds[@]}; do
        ...
        if [[ ! -e "$file" ]]; then
            ...
            CUDA_VISIBLE_DEVICES=${gpus[${gpu_index}]} python train.py --data_path data/${datasets[$i]}.csv --dataset_type ${dataset_type[$i]} --save_dir ../ckpt/417_hyperopt_ensemble/${datasets[$i]}/random/${folds[$fold]} ${split_info} --features_path /data/rsg/chemistry/yangk/saved_features/${datasets[$i]}.pckl --no_features_scaling --config_path ../ckpt/417_hyperopt/${datasets[$i]}/random/${folds[$fold]}/config.json --quiet --metric ${metrics[$i]} --ensemble_size 5 &
            gpu_index=$(($((${gpu_index} + 1)) % ${num_gpus}))
        fi
    done
    wait
